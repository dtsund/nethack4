...
#define SPELL(name,desc,sub,prob,delay,level,mgc,dir,color) OBJECT( \
    ...
    SPELL("dig", "parchment", P_ESCAPE_SPELL, 20, 6, 5, 1, RAY, HI_PAPER),
    SPELL("magic missile", "vellum", P_ATTACK_SPELL, 50, 2, 2, 1, RAY,
          HI_PAPER),
    SPELL("fireball", "ragged", P_ATTACK_SPELL, 25, 4, 4, 1, RAY, HI_PAPER),
    SPELL("cone of cold", "dog eared", P_ATTACK_SPELL, 15, 7, 4, 1, RAY,
          HI_PAPER),
    SPELL("sleep", "mottled", P_ENCHANTMENT_SPELL, 55, 1, 4, 1, RAY, HI_PAPER),
    SPELL("finger of death", "stained", P_ATTACK_SPELL, 5, 10, 7, 1, RAY,
          HI_PAPER),
    SPELL("light", "cloth", P_DIVINATION_SPELL, 50, 1, 1, 1, NODIR, HI_CLOTH),
    SPELL("detect monsters", "leather-bound", P_DIVINATION_SPELL, 43, 1, 2, 1,
          NODIR, HI_LEATHER),
    SPELL("healing", "white", P_DEFENSE_SPELL, 45, 2, 1, 1, IMMEDIATE,
          CLR_WHITE),
    SPELL("knock", "pink", P_ESCAPE_SPELL, 40, 1, 1, 1, IMMEDIATE,
          CLR_BRIGHT_MAGENTA),
    SPELL("force bolt", "red", P_ATTACK_SPELL, 40, 2, 1, 1, IMMEDIATE, CLR_RED),
    SPELL("confuse monster", "orange", P_ENCHANTMENT_SPELL, 30, 2, 2, 1,
          IMMEDIATE, CLR_ORANGE),
    SPELL("drain life", "velvet", P_MATTER_SPELL, 10, 2, 2, 1, IMMEDIATE,
          CLR_MAGENTA),
    SPELL("slow monster", "light green", P_ENCHANTMENT_SPELL, 35, 2, 1, 1,
          IMMEDIATE, CLR_BRIGHT_GREEN),
    SPELL("wizard lock", "dark green", P_ESCAPE_SPELL, 35, 3, 2, 1, IMMEDIATE,
          CLR_GREEN),
    SPELL("create monster", "turquoise", P_MATTER_SPELL, 40, 3, 3, 1, NODIR,
          CLR_BRIGHT_CYAN),
    SPELL("cause fear", "light blue", P_ENCHANTMENT_SPELL, 25, 3, 3, 1, NODIR,
          CLR_BRIGHT_BLUE),
    SPELL("clairvoyance", "dark blue", P_DIVINATION_SPELL, 15, 3, 6, 1, NODIR,
          CLR_BLUE),
    SPELL("cure sickness", "indigo", P_DEFENSE_SPELL, 37, 3, 2, 1, NODIR,
          CLR_BLUE),
    SPELL("charm monster", "magenta", P_ENCHANTMENT_SPELL, 20, 3, 6, 1,
          IMMEDIATE, CLR_MAGENTA),
    SPELL("haste self", "purple", P_ESCAPE_SPELL, 38, 4, 3, 1, NODIR,
          CLR_MAGENTA),
    SPELL("detect unseen", "violet", P_DIVINATION_SPELL, 20, 4, 2, 1, NODIR,
          CLR_MAGENTA),
    SPELL("levitation", "tan", P_MATTER_SPELL, 20, 4, 1, 1, NODIR, CLR_BROWN),
    SPELL("extra healing", "plaid", P_DEFENSE_SPELL, 27, 5, 3, 1, IMMEDIATE,
          CLR_GREEN),
    SPELL("invisibility", "dark brown", P_ESCAPE_SPELL, 25, 5, 4, 1, NODIR,
          CLR_BROWN),
    SPELL("detect treasure", "gray", P_DIVINATION_SPELL, 20, 5, 4, 1, NODIR,
          CLR_GRAY),
    SPELL("remove curse", "wrinkled", P_MATTER_SPELL, 30, 5, 4, 1, NODIR,
          HI_PAPER),
    SPELL("magic mapping", "dusty", P_DIVINATION_SPELL, 23, 7, 5, 1, NODIR,
          HI_PAPER),
    SPELL("identify", "bronze", P_DIVINATION_SPELL, 20, 6, 4, 1, NODIR,
          HI_COPPER),
    SPELL("turn undead", "copper", P_DEFENSE_SPELL, 16, 8, 4, 1, IMMEDIATE,
          HI_COPPER),
    SPELL("polymorph", "silver", P_MATTER_SPELL, 10, 8, 6, 1, IMMEDIATE,
          HI_SILVER),
    SPELL("teleport away", "gold", P_ESCAPE_SPELL, 15, 6, 6, 1, IMMEDIATE,
          HI_GOLD),
    SPELL("create familiar", "glittering", P_DEFENSE_SPELL, 10, 7, 6, 1, NODIR,
          CLR_WHITE),
    SPELL("cancellation", "shining", P_ENCHANTMENT_SPELL, 15, 8, 5, 1, IMMEDIATE,
          CLR_WHITE),
    SPELL("protection", "dull", P_DEFENSE_SPELL, 18, 3, 1, 1, NODIR, HI_PAPER),
    SPELL("jumping", "thin", P_ESCAPE_SPELL, 25, 3, 1, 1, IMMEDIATE, HI_PAPER),
    SPELL("stone to flesh", "thick", P_MATTER_SPELL, 15, 1, 3, 1, IMMEDIATE,
          HI_PAPER),
// Some null spellbooks to retain the old IDs.  We have them, might as well
// use them.
    SPELL(NULL, "cyan", P_NONE, 0, 0, 0, 0, 0, CLR_CYAN),
    SPELL(NULL, "yellow", P_NONE, 0, 0, 0, 0, 0, CLR_YELLOW),
    SPELL(NULL, "light brown", P_NONE, 0, 0, 0, 0, 0, CLR_BROWN),
